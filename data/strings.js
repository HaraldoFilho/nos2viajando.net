var strings_dict = {
  'TITLE': 'Nós Dois Viajando',
  'TRIPS': 'viagens',
  'TRIP': 'viagem',
  'FLIGHTS': 'voos',
  'COUNTRIES': 'países',
  'COUNTRY': 'país',
  'CONTINENTS': 'continentes',
  'EU': 'Europa',
  'WW': 'Mundo',
  'HALF_DAY': 'bate-volta',
  'DAY': 'dia',
  'DAYS': 'dias',
  'HOURS': 'horas',
  'BLOG': 'IR PARA O DIÁRIO DE VIAGEM',
  'CITIES': 'Cidades',
  'PARKS': 'Parques Nacionais',
  'ATTRACTIONS': 'Atrações Turísticas',
  'ACCOMMODATIONS': 'Hospedagens',
  'RESTAURANTS': 'Alimentação',
  'FAVORITE': 'Favorito',
  'AIRPORTS': 'Aeroportos',
  'PORTS': 'Portos',
  'STATIONS': 'Estações de Trem',
  'NORTH': 'Norte',
  'SOUTH': 'Sul',
  'EAST': 'Leste',
  'WEST': 'Oeste',
  'TRIPS_LOCAL': ' locais',
  'TRIPS_DOMESTIC': ' domésticas',
  'TRIPS_INTERNATIONAL': ' internacionais',
  'FLIGHTS_DOMESTIC': ' domésticos',
  'FLIGHTS_INTERNATIONAL': ' internacionais',
  'FLIGHT_CONNECT': ' (conexão de voo)',
  'MAP_ITEMS': 'Items do Mapa',
  'MAP_PROJECTION_1': 'mapa',
  'MAP_PROJECTION_2': 'globo',
  'MAP_LAYER_1': 'trânsito',
  'MAP_LAYER_2': 'relevo',
  'MAP_LAYER_3': 'satélite',
  'MKR_CITIES': 'cidades',
  'MKR_PARKS': 'parques',
  'MKR_ATTRACTIONS': 'atrações',
  'MKR_ACCOMMODATIONS': 'hospedagens',
  'MKR_RESTAURANTS': 'alimentação',
  'MKR_AIRPORTS': 'aeroportos',
  'MKR_PORTS': 'portos',
  'MKR_STATIONS': 'estações de trem',
  'MKR_PHOTOS': 'fotos',
  'MKR_INTERNATIONAL_FLIGHT': 'voos internacionais',
  'MKR_DOMESTIC_FLIGHT': 'voos domésticos',
  'MKR_BUS': 'ônibus',
  'MKR_TRAIN': 'trem',
  'MKR_BOAT': 'embarcação',
  'MKR_CAR': 'viagens de carro',
  'MKR_CAR_ABROAD': 'carro alugado',
  'MKR_FARTHEST_POINTS': 'extremos',
}

var strings_dict_en = {
  'TITLE': 'We Traveling',
  'TRIPS': 'trips',
  'TRIP': 'trip',
  'FLIGHTS': 'flights',
  'COUNTRIES': 'countries',
  'COUNTRY': 'country',
  'CONTINENTS': 'continents',
  'EU': 'Europe',
  'WW': 'World',
  'HALF_DAY': 'day trip',
  'DAY': 'dia',
  'DAYS': 'days',
  'HOURS': 'hours',
  'BLOG': 'GO TO THE TRAVEL DIARY',
  'CITIES': 'Cities',
  'PARKS': 'National Parks',
  'ATTRACTIONS': 'Touristic Attractions',
  'ACCOMMODATIONS': 'Accommodations',
  'RESTAURANTS': 'Food',
  'FAVORITE': 'Favorite',
  'AIRPORTS': 'Airports',
  'PORTS': 'Ports',
  'STATIONS': 'Train Stations',
  'NORTH': 'North',
  'SOUTH': 'South',
  'EAST': 'East',
  'WEST': 'West',
  'TRIPS_LOCAL': ' locals',
  'TRIPS_DOMESTIC': ' domestics',
  'TRIPS_INTERNATIONAL': ' internationals',
  'FLIGHTS_DOMESTIC': ' domestics',
  'FLIGHTS_INTERNATIONAL': ' internationals',
  'FLIGHT_CONNECT': ' (flight connection)',
  'MAP_ITEMS': 'Map Items',
  'MAP_PROJECTION_1': 'map',
  'MAP_PROJECTION_2': 'globe',
  'MAP_LAYER_1': 'streets',
  'MAP_LAYER_2': 'elevation',
  'MAP_LAYER_3': 'satellite',
  'MKR_CITIES': 'cities',
  'MKR_PARKS': 'parks',
  'MKR_ATTRACTIONS': 'attractions',
  'MKR_ACCOMMODATIONS': 'accommodations',
  'MKR_RESTAURANTS': 'food',
  'MKR_AIRPORTS': 'airports',
  'MKR_PORTS': 'ports',
  'MKR_STATIONS': 'train stations',
  'MKR_PHOTOS': 'photos',
  'MKR_INTERNATIONAL_FLIGHT': 'international flights',
  'MKR_DOMESTIC_FLIGHT': 'domestic flights',
  'MKR_BUS': 'bus',
  'MKR_TRAIN': 'train',
  'MKR_BOAT': 'vessel',
  'MKR_CAR': 'road trips',
  'MKR_CAR_ABROAD': 'rented car',
  'MKR_FARTHEST_POINTS': 'extremes'
}
